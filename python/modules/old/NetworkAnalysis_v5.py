...
class NetworkAnalysis:
    ...
    def _edge_list(self, data, column_, column_split):
        ...
        for co_word in data2[column_]:
            # Get all combinations of co-words (pairs)
            co_word_combinations = list(itertools.combinations(co_word, 2))
            # Add unique combinations (edges) to the co_words_list >> to reflect undirected network
            for pair in co_word_combinations:
                if pair not in co_words_list and tuple(reversed(pair)) not in co_words_list:
                    co_words_list.append(pair)
