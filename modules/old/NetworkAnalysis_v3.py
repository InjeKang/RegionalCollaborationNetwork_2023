...
def knowledge_network(column_, descriptive, toEdgeList):
    ...
    if descriptive:        
        ...
        #     if column_ == "region":
        #     # generation trend
        #         # Group by year and count the number of patents - region
        #         patents_by_year = sub_data.groupby("year").size()
        #         df_merged_overall.append(patents_by_year)


        #     if column_ == "app_reg":
        #         # Group by year and count the number of patents - applicants
        #         sub_data2 = sub_data.assign(app_reg=sub_data['app_reg'].str.split('|')).explode('app_reg')
        #         sub_data2 = sub_data2[sub_data2["app_reg"].str.contains(region, na=False)]
        #         patents_by_year = sub_data2.groupby(['year', 'app_reg']).size().reset_index(name='count')
        #         # Sort the firm counts within each year in descending order
        #         patents_by_year_sorted = patents_by_year.groupby('year').apply(lambda x: 
        #                                                                        x.sort_values(by='count', ascending=False)).reset_index(drop=True)
        #         top_10_firms = patents_by_year_sorted.groupby('year').head(20).reset_index()
        #         df_merged_overall.append(top_10_firms)
        ...
        for region in region_list:            
            ...
            if column_ == "app_reg":
                sub_data["collab"] = sub_data["region"].apply(lambda x: "공동" if "|" in x else "단독")
                sub_data = sub_data[sub_data["collab"].str.contains("공동")]
                sub_data2 = sub_data.assign(app_reg=sub_data['app_reg'].str.split('|')).explode('app_reg')
                sub_data2 = sub_data2[sub_data2["app_reg"].str.contains(region, na=False)]
                patents_by_year = sub_data2.groupby(['year', 'app_reg']).size().reset_index(name='count')
                # Sort the firm counts within each year in descending order
                patents_by_year_sorted = patents_by_year.groupby('year').apply(lambda x: 
                                                                                x.sort_values(by='count', ascending=False)).reset_index(drop=True)
                top_10_firms = patents_by_year_sorted.groupby('year').head(20).reset_index()
                df_merged_collab.append(top_10_firms) 
