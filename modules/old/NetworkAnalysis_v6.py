...
class NetworkAnalysis:
    ...
    def collaboration_network(self, type_, run): # type_ = collab OR knowledge
        if run:
            if self.doc_type == "paper":
                ...
                sub_data2 = data[data["collab"].str.contains("sole")]

                
                if type_["collab"]:
                    # edge list, adjacency matrix, and visualization
                    self._network_analysis_compilation(sub_data, "affiliation_region", ";", "collab") # data, column_, column_split, collab_
                    # measuring centrality
                    result = self._measure_centrality(sub_data, "affiliation_region", ";", "overall", "collab") # data, column_, column_split, region_, collab_
                    for region in FilterData().region_list:
                        sub_data_byRegion = data[data["affiliation_region"].str.startswith(region)]
                        result = self._measure_centrality(sub_data_byRegion, "affiliation_region", ";", region, "collab") # data, column_, column_split, region_, collab_    
                ...
                if type_["knowledge"]:        
                    ...
                    self._network_analysis_compilation(sub_data, "keywords", ";", "collab") # data, column_, column_split, collab_
                    # measuring centrality
                    result = self._measure_centrality(sub_data, "keywords", ";", "overall", "collab") # data, column_, column_split, region_, collab_
                    for region in FilterData().region_list:
                        sub_data_byRegion = data[data["affiliation_region"].str.startswith(region)]
                        result = self._measure_centrality(sub_data_byRegion, "keywords", ";", region, "collab") # data, column_, column_split, region_, collab_

                    # sole papers
                    # edge list, adjacency matrix, and visualization                    
                    sub_data2["keywords"] = sub_data2["keywords"].str.lower()
                    self._network_analysis_compilation(sub_data2, "keywords", ";", "sole") # data, column_, column_split, collab_
                    # measuring centrality
                    result = self._measure_centrality(sub_data2, "keywords", ";", "overall", "sole") # data, column_, column_split, region_, collab_
                    for region in FilterData().region_list:
                        sub_data_byRegion = data[data["affiliation_region"].str.startswith(region)]
                        result = self._measure_centrality(sub_data_byRegion, "keywords", ";", region, "sole") # data, column_, column_split, region_, collab_

                return result
                
    def _measure_centrality(self, data, column_, column_split, region_, collab_):
        ...
        if column_ == "keywords" :
            ...
            filter_edgeList_subset = filter_edgeList[filter_edgeList["freq"] > 10]
        ...
        node_freq = {(a, b, len(g)) for (a, b), g in df_edgeList.groupby(['column1', 'column2'])}
        G = nx.Graph()
        ...
        mergedDF.to_excel(f"data\\07.NetworkResults_{column_}_{region_}__{collab_}.xlsx", index=False)
        return mergedDF    

    def _network_analysis_compilation(self, data, column_, column_split, collab_):
        ...
        # save the results
        with pd.ExcelWriter(f"data\\06.{self.doc_type}_{column_}_Network_AdjMat_{collab_}.xlsx") as writer:
            df_adjacency_matrix.to_excel(writer, sheet_name="overall", index=False)
            for i in trange(0, len(FilterData().region_list)):
                region_AdjMat[i].to_excel(writer, sheet_name = str(FilterData().region_list[i]), index=False)

        with pd.ExcelWriter(f"data\\06.{self.doc_type}_{column_}_Network_EdgeList_{collab_}.xlsx") as writer:
    ...
    def _adjacency_matrix(self, data, column_, column_split):
        ...
        nodes = set(pd.concat([filter_edgeList_subset["column1"], filter_edgeList_subset["column2"]]))
        ...
        for row in filter_edgeList_subset.itertuples(index=False):
            col1, col2, freq = row
            to_adjacency_matrix.loc[col1, col2] += freq
            to_adjacency_matrix.loc[col2, col1] += freq
        return filter_edgeList_subset, to_adjacency_matrix
